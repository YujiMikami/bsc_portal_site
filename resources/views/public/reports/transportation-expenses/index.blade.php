            ...
            <div class="bg-white shadow-sm rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="flex justify-start mb-4">
                        <a href="{{ route('public.reports.transportation-expenses.create') }}"
                                ...
                                @foreach ($expensesByDate as $submittedDate => $expenses)
                                    <tr>
                                        <td class="border px-4 py-2">
                                            {{ $expenses->first()->created_at->format('Y-m-d') }}</td>
                                        @foreach ($expenses as $expense)
                                            @if ($loop->first)
                                                <td class="border px-4 py-2">
                                                    {{ explode('_', $submittedDate)[0] == '1000-01-01' ? '未申請' : explode('_', $submittedDate)[0] }}
                                                </td>
                                                <td class="border px-4 py-2">{{ $expense->employee_id }}</td>
                                                <td class="border px-4 py-2">
                                                    {{ $expense->EmployeeAccount->employee_name }}</td>
                                                <td class="border px-4 py-2">
                                                    {{ number_format($expenses->sum('amount')) }}円</td>
                                                <td class="border px-4 py-2">
                                                ...
                                                <td class="border px-4 py-2">
                                                    <div
                                                        ...
                                                        {{-- 詳細ボタンを追加 --}}
                                                        <a href="{{ route('public.reports.transportation-expenses.show', ['id' => $expense->employee_id, 'date' => explode('_', $submittedDate)[0]]) }}"
                                                            class="text-blue-600 hover:underline">詳細</a>
                                                        {{-- 編集ボタンを追加 --}}
                                                        @can('view', $expense)
                                                            {{-- policyを使って制御 --}}
                                                            @if (empty($expense->submitted))
                                                                <a href="{{ route('public.reports.transportation-expenses.edit', ['id' => $expense->employee_id, 'date' => explode('_', $submittedDate)[0]]) }}"
                                                                    class="text-green-600 hover:underline">編集</a>
                                                                {{-- 削除ボタンを追加 --}}
                                                                <form
                                                                    action="{{ route('public.reports.transportation-expenses.delete', ['id' => $expense->employee_id, 'date' => explode('_', $submittedDate)[0]]) }}"
                                                        ...
                                                        @can('approval', $expense)
                                                            @if (empty($expense->approver))
                                                                <form
                                                                    action="{{ route('public.reports.transportation-expenses.approval', ['id' => $expense->employee_id, 'date' => explode('_', $submittedDate)[0]]) }}"
                                                                    method="POST"
                                                                    onsubmit="return confirm('有給申請を許可しますか？');">
                                                                    @csrf
                                                                    @method('PUT')
                                                                    <button type="submit"
                                                                        class="text-red-600 hover:underline bg-transparent border-none cursor-pointer p-0 m-0">承認</button>
                                                                </form>
                                                            @endif
                                                        @endcan
                                                        @can('acceptance', $expense)
                                                            @if (isset($expense->approver) && empty($expense->recipient))
                                                                <form
                                                                    action="{{ route('public.reports.transportation-expenses.acceptance', ['id' => $expense->employee_id, 'date' => explode('_', $submittedDate)[0]]) }}"
                                                                    method="POST"
                                                                    onsubmit="return confirm('有給申請を受理しますか？');">
                                                                    @csrf
                                                                    @method('PUT')
                                                                    <button type="submit"
                                                                        class="text-red-600 hover:underline bg-transparent border-none cursor-pointer p-0 m-0">受理</button>
                                                                </form>
                                                            @endif
                                                        @endcan
                                                    </div>
