        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('public.reports.paid-requests.store') }}" method="POST">
                        @csrf
                        
                        <div class="mb-4 flex">
                            <div class="mb-4 mr-3">
                                <label for="application_date" class="block text-gray-700 text-sm font-bold mb-2">申請日：</label>
                                <input type="date" name="application_date" id="application_date" value="{{ old('application_date', \Carbon\Carbon::parse(now())->format('Y-m-d')) }}" class="shadow appearance-none border rounded  py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                            </div>
                        ...
                        <div class="mb-4 flex">
                            <div class="mb-4 mr-3">
                                ...
                                <input type="date" name="start_date" id="start_date" value="{{ old('start_date', '') }}" class="shadow appearance-none border rounded  py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                            </div>

                            <div class="mb-4 mr-3">
                                <label for="end_date" class="block text-gray-700 text-sm font-bold mb-2">終了日：</label>
                                <input type="date" name="end_date" id="end_date" value="{{ old('end_date', '') }}" class="shadow appearance-none border rounded  py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                            </div>

                            <div class="mb-4 mr-3">
                                <label for="days" class="block text-gray-700 text-sm font-bold mb-2 ">日数</label>
                                <input type="text" name="days" id="days" size="2" value="{{ old('days', '') }}" class="shadow appearance-none border rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">日間
                            </div>
                        </div>
                        <div class="mb-6">
                            ...
                            <select name="distinction" id="distinction" class="rounded leading-tight">
                                @foreach (config('const.paid_distinction') as $val)
                                    <option value="{{ $val }}"{{ old('distinction', '') == $val ? 'selected ' : '' }}>
                                        {{ $val }}
                                    </option>
                                @endforeach
                            </select>   
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="reason" id="reason" size="30" value="{{ old('reason', '') }}" class="shadow appearance-none border rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        </div>

                        <div class="mb-4">
                            <label for="note" class="block text-gray-700 text-sm font-bold mb-2 ">備考</label>
                            <input type="text" name="note" id="note" size="30" value="{{ old('note', '') }}" class="shadow appearance-none border rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        </div>
